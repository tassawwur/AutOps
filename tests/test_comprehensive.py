            ...
            with patch.object(agent, 'datadog_client') as mock_dd:
                with patch.object(agent, 'pagerduty_client') as mock_pd:
                    with patch.object(agent, 'gitlab_client') as mock_gl:
                        mock_dd.get_error_rate_metrics.return_value = {"error_rate": "1.5%"}
...
class TestToolExecutionAgent:
    ...
    def test_execute_step_github_client(self, agent):
        ...
        with patch('src.autops.tools.github_client.get_latest_pipeline_status') as mock_github:
...
class TestVerificationAgent:
    ...
    async def test_reflect_on_workflow(self, agent):
        ...
        plan = {
            "intent": "investigate_incident",
            "original_query": "Service is down"
        }
        
        execution_results = [
            {
                "status": "completed",
                "result": {"analysis": "High error rate detected"}
            }
        ]
        ...
        with patch.object(agent, 'client') as mock_client:
            ...
            mock_response.choices[0].message.content = '{"overall_success": true, "confidence_score": 0.9, "insights": {"successes": ["Data gathered"], "failures": [], "unexpected_findings": []}, "recommendations": {"immediate_actions": [], "future_improvements": []}, "risk_assessment": {"risk_level": "low", "risk_factors": []}}'
        ...
        with patch('src.autops.agents.query_understanding_agent.client') as mock_openai:
            with patch('src.autops.tools.github_client.get_latest_pipeline_status') as mock_github:
                with patch('src.autops.tools.slack_client.slack_client') as mock_slack:
                    # Setup mocks
                    mock_openai.chat.completions.create.return_value.choices[0].message.content = '{"intent": "get_ci_cd_status", "entities": {"service_name": "test-service"}, "confidence": 0.9}'
                    mock_github.return_value = {"status": "success", "conclusion": "success"}
                    mock_slack.post_message = AsyncMock()
                    
                    # Run workflow components
                    agent = QueryUnderstandingAgent()
                    query = await agent.get_structured_query("Is test-service build passing?")
                    plan = create_plan(query)
...
class TestPerformance:
    ...
    async def test_query_understanding_performance(self):
        ...
        with patch.object(agent, '_call_openai_api') as mock_api:
            mock_api.return_value = '{"intent": "get_ci_cd_status", "entities": {}, "confidence": 0.9}'
