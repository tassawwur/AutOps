...
class TestAgentOrchestration:
    ...
    async def test_full_query_workflow_ci_status(self, orchestrator):
        ...
        with patch.object(orchestrator.query_agent, 'understand_query') as mock_understand, \
             ...
             patch.object(orchestrator.response_agent, 'generate_response') as mock_response:
    ...
    async def test_concurrent_query_processing(self, orchestrator):
        ...
        with patch.object(orchestrator.query_agent, 'understand_query') as mock_understand, \
            ...
            mock_understand.return_value = {"intent": "test", "entities": {}, "confidence": 0.9}
            mock_plan.return_value = {"steps": [{"action": "test", "tool": "github", "parameters": {}}]}
            ...
            assert all(isinstance(r, dict) and r["status"] == "success" for r in results)
    ...
    async def test_knowledge_gap_handling(self, orchestrator):
        ...
        with patch.object(orchestrator.query_agent, 'understand_query') as mock_understand, \
             patch.object(orchestrator.info_retrieval_agent, 'search_knowledge_base') as mock_search, \
             patch.object(orchestrator.response_agent, 'generate_response') as mock_response:
            ...
            mock_response.return_value = {
                ...
                "confidence": 0.85,
                "sources": ["Incident Report - Database Outage", "Post-mortem Analysis"]
            }
