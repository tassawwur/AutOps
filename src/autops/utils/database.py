...
from sqlalchemy import (
    create_engine, Column, Integer, String, DateTime, JSON, Text, Boolean, 
    ForeignKey, Index, desc, and_, or_
)
...
class Query(Base):
    ...
    status = Column(String(50), nullable=False, default="processing")  # processing, completed, error
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow)
    completed_at = Column(DateTime, nullable=True)
    execution_time_ms = Column(Integer, nullable=True)
    
    # Relationships
    agent_executions = relationship("QueryAgentExecution", back_populates="query", cascade="all, delete-orphan")
...
class QueryAgentExecution(Base):
    ...
    __table_args__ = (
        Index('idx_query_agent_exec_query', 'query_id'),
    )
...
class ServiceMetrics(Base):
    ...
    metric_type = Column(String(100), nullable=False)  # error_rate, latency, throughput, etc.
    ...
    __table_args__ = (
        Index('idx_service_metrics_service_type_time', 'service_name', 'metric_type', 'timestamp'),
...
class DatabaseManager:
    ...
    def initialize(self, database_url: str = None):
        ...
        try:
            ...
            logger.info("Database initialized successfully", url=db_url.split("@")[-1] if "@" in db_url else db_url)
...
class QueryRepository:
    ...
    @staticmethod
    def update_query_status(session: Session, query_id: str, status: str, **kwargs) -> bool:
    ...
    @staticmethod
    def get_recent_queries(session: Session, user_id: str = None, limit: int = 50) -> List[Query]:
...
class MetricsRepository:
    ...
    @staticmethod
    def store_metrics(session: Session, service_name: str, metric_type: str, 
                     value: Dict[str, Any], source: str, record_metadata: Dict[str, Any] = None):
    ...
    @staticmethod
    def get_metrics(session: Session, service_name: str, metric_type: str = None,
                   start_time: datetime = None, end_time: datetime = None) -> List[ServiceMetrics]:
        """Get metrics with filtering."""
        query = session.query(ServiceMetrics).filter(ServiceMetrics.service_name == service_name)
    ...
    def cleanup_old_metrics(session: Session, days_to_keep: int = 30) -> int:
        ...
        deleted = session.query(ServiceMetrics).filter(
            ServiceMetrics.timestamp < cutoff_date
        ).delete()
        return deleted
...
class IncidentRepository:
    ...
    @staticmethod
    def get_active_incidents(session: Session, service_name: str = None) -> List[Incident]:
        """Get active incidents."""
        query = session.query(Incident).filter(Incident.status.in_(["open", "investigating"]))
        if service_name:
            query = query.filter(Incident.service_name == service_name)
        return query.order_by(desc(Incident.created_at)).all()
    
    @staticmethod
    def resolve_incident(session: Session, incident_id: str, 
                        resolution_actions: Dict[str, Any] = None, auto_resolved: bool = False) -> bool:
        """Resolve an incident."""
...
class KnowledgeBaseRepository:
    ...
    @staticmethod
    def search_articles(session: Session, query: str, category: str = None, 
                       service_name: str = None, limit: int = 10) -> List[KnowledgeBase]:
        """Search knowledge base articles."""
        search_query = session.query(KnowledgeBase).filter(KnowledgeBase.is_active == True)
