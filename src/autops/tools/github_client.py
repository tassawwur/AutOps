...
class GitHubClient:
    ...
    def _get_repository(self, repo_name: str) -> Repository:
        ...
        try:
            ...
            if e.status == 404:
                raise GitHubAPIError(f"Repository '{repo_name}' not found in '{self.owner}'")
            else:
    ...
    def get_latest_pipeline_status(self, repo_name: str, branch: str = None) -> Dict[str, Any]:
        """
            ...
            if workflow_runs.totalCount == 0:
                ...
                try:
                    ...
                    for run in recent_runs:
                        ...
                        summary["by_conclusion"][conclusion] = summary["by_conclusion"].get(conclusion, 0) + 1
                    
                    pipeline_data["workflow_summary"] = summary
                except Exception as e:
                    self.logger.warning("Failed to generate workflow summary", error=str(e))
    ...
    def get_recent_commits(self, repo_name: str, branch: str = None, days: int = 7) -> Dict[str, Any]:
        """
    ...
    def get_pull_requests(self, repo_name: str, state: str = "open", limit: int = 20) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                repo_data["recent_activity"] = {
                    ...
                    "last_commit_date": recent_commits[0].commit.author.date.isoformat() 
                        if recent_commits and recent_commits[0].commit.author.date else None
                }
            except Exception as e:
...
def get_github_client() -> GitHubClient:
    ...
    if _github_client is None:
        _github_client = GitHubClient(token=settings.github_token, owner=settings.github_owner)
